...
def Offer(offered_organ, matching_recipient, noffers, Sim, Patients_Accept, Donor_Accept, DSA_Avg_Times, AcceptanceModelS1, AcceptanceModel):
    ...
    Input:
        ...
        @DSA_Avg_Times: data on average transport time between DSAs
        @AcceptanceModelS1: coefficients regarding patient's characteristics for status-1 acceptance model
        @AcceptanceModel: coefficients regarding patient's characteristics for non-status-1
    Output:
    ...
    if config.REDUCED_MODEL:
        REDUCED_MODEL = nump.loadtxt('../FakeInput/Input_Acceptance_Reduced.txt')
        patientx = [1,
                    matching_recipient.Status1,
                    int(matching_recipient.DSA == offered_organ.DSA),
                    int(1 == offered_organ.ABO),
                    int(2 == offered_organ.ABO),
                    (Sim.clock - matching_recipient.create_time)
                    ]
        accept_prob = nump.exp(nump.dot(patientx, REDUCED_MODEL)) / (
                    1 + nump.exp(nump.dot(patientx, REDUCED_MODEL)))
